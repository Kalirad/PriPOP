...
def sigmoid(x, state, strain, diet):
    """Function to claculate the transition probability between developmental stages.

    Args:
        x (int): current age in hours.
        state (str): current developmental state.
        strain (str): strain of the worm.
        diet (str): bacterial diet.
...
def create_neighbor_dict(line_length, ring=False):
    """Create a dicitionary to specified the relationship between neighboring populations.

    Args:
        line_length (int): number of populations in the metapopulation.
        ring (bool, optional):If true, specifies a ring neighborhood. Defaults to False.
...
def assign_items_to_neighbors(items, neighbors, curr):
    """Adds worms to populations in a metapopulation.

    Args:
        items (list): list of dauer larvae.
        neighbors (list): list of neighboring populations
        curr (int): index of current population.

    Returns:
        list : list of assigned dauer larvae.
    """
    if len(neighbors) == 1:
...
def migration(possible, neighbours, mig_rate):
    """Migrate dauer larvae across metapopulation.

    Args:
        possible (list): list of potential migrating dauer larvae.
        neighbours (list): list of neighboring populations.
        mig_rate (float): migration probability.
...
class Population(object):
    ...
    def __init__(self, dim, resource_type='OP50', rw_l=5, walk_bias=(1, 1), recog_par=0.95, pred_par=0.1, mig_rate=0.05, egg_var=True, loc_save=False):
        """Initialize an instance of population object.

        Args:
            dim (tuple): dimensions of the lattice.
            resource_type (str, optional): bacterial diet ('Novo' or 'OP50'). Defaults to 'OP50'.
            rw_l (int, optional): max length of random walk step. Defaults to 5.
            walk_bias (tuple, optional): random walk bias. Defaults to (1, 1).
            recog_par (float, optional): kin recognition fidelity. Defaults to 0.95.
            pred_par (float, optional): probability of killing a pre-adult larvae upon encounter during random walk. Defaults to 0.1.
            mig_rate (float, optional): probability of dauer migration at each step of simulation. Defaults to 0.05.
            egg_var (bool, optional): if true, fecundity between worms is entirely based on the ppd generated by the bayesian model. Defaults to True.
            loc_save (bool, optional): if true, saves the location of worms during the simulation. Defaults to False.
        """
    ...
    def empty_loc_top(self):
        """return the empty locations on the top lattice.
    ...
    def empty_loc_middle(self):
        """return the empty locations on the middle lattice.
    ...
    def empty_loc_bottom(self):
        """return the empty locations on the bottom lattice.
    ...
    def random_walk_2d(self, loc, empty_spots, pot_predator_spots=False):
        """Simulate random walk over the lattice

        Args:
            loc (tuple): current location.
            empty_spots (list): list of the empty spots.
            pot_predator_spots (bool, optional): If true, worm avoids spots occupied by predatory adults. Defaults to False.

        Returns:
            list: list of locations traversed during random walk.
        """
        step_taken = 0
        trajectory = []
        curr_loc = loc
        ...
        while step_taken < self.rw_l:
            ...
            if len(curr_neighbors) == 0:
                ...
                if not pot_predator_spots:
                    next_loc = curr_neighbors[rnd.randint(0, len(curr_neighbors) - 1)]
                else:
                    w_steps = [self.walk_bias[0] if i not in pot_predator_spots else self.walk_bias[1] for i in curr_neighbors]
                    if np.sum(w_steps) == 0:
                        break
                    else:
    ...
    def move_worms(self):
        """Execute all the possible random walk moves.
        """
    ...
    def transfer_layers(self, top, mid, bottom):
        """Replace the layers of the three lattices with new ones.

        Args:
            top (dictionary): new top lattice
            mid (dictionary): new middle lattice
            bottom (dictionary): new bottom lattice
        """
        self.top_layer = top
    ...
    def add_worm(self, worm):
        """Add a worm to the approperiate lattice.

        Args:
            worm (tupple): a worm with age, strain, and developmental stage data.
        """
        if not worm[4]:
    ...
    def lay_eggs(self, age_lim=120):
        """Allow hermaphrodites to lay eggs.

        Args:
            age_lim (int, optional): The oldest age at which a hermaphrodite could lay eggs. Defaults to 120.
        """
    ...
    def migrate_dauer(self):
        """Migrate dauer larvae out of the population.
        """
        P_migrants = 0
        NP_migrants = 0
        MP_migrants = 0
    ...
    def age_pop(self):
        """Add one hour to the age of every worm in the population.
        """
    ...
    def take_a_step(self):
        """Execuate a step of the simulation.
        """
    ...
    def simulate(self, t, verbose=False):
        ...
        Args:
            t (int): length of simulation in hours.
